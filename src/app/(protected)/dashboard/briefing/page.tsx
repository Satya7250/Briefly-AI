...
export default function BriefingPage() {
  ...
  return (
    <div className="flex flex-col h-full min-h-screen bg-[#09090B] p-4 md:p-6 gap-6 transition-all duration-300 text-foreground">
      ...
      {messages.length === 0 ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 shrink-0 transition-all duration-500 ease-out animate-in fade-in slide-in-from-top-4">
            {aiCards.map((card, idx) => (
              <Card 
                key={card.title} 
                className="flex flex-col border border-[#27272A] shadow-[0_2px_8px_-3px_rgba(0,0,0,0.05)] hover:shadow-md hover:border-[#8B5CF6]/30 transition-all duration-300 bg-[#111113] rounded-2xl"
          ...
          <Card className="flex flex-col h-[calc(100vh-320px)] min-h-[400px] border border-[#27272A] shadow-sm overflow-hidden bg-[#111113]/30 backdrop-blur-sm rounded-2xl">
            <CardHeader className="pb-3 pt-4 px-6 border-b border-[#27272A] shrink-0 select-none">
            ...
            <CardContent className="flex-1 flex flex-col p-0 overflow-hidden">
                ...
                <div className="h-full flex flex-col items-center justify-center py-6 text-center space-y-7">
                  <div className="flex flex-col items-center space-y-3.5 max-w-md animate-in fade-in duration-300">
                    <div className="relative">
                      <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-[#8B5CF6] to-indigo-500 opacity-20 blur-md animate-pulse" />
                      <div className="relative p-4.5 rounded-full bg-gradient-to-tr from-[#8B5CF6] to-indigo-600 text-white shadow-lg">
                        <SparklesIcon className="size-7 animate-[spin_4s_linear_infinite]" />
                      </div>
                    </div>
                    <h2 className="text-xl font-bold tracking-tight text-white">Meet Briefly</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full max-w-2xl mt-3">
                    {suggestionPrompts.map((prompt, idx) => (
                      <button
                        key={prompt.text}
                        className="flex items-center justify-between p-3.5 rounded-xl border border-[#27272A] bg-[#111113] hover:bg-[#8B5CF6]/5 hover:border-[#8B5CF6]/35 hover:shadow-sm text-left text-xs md:text-sm transition-all duration-300 group hover:-translate-y-0.5 cursor-pointer"
                        style={{ animationDelay: `${idx * 50}ms` }}
                        onClick={() => sendMessage(prompt.text)}
                      >
                        <div className="flex items-center gap-3 pr-2">
                          <span className="flex items-center justify-center w-7 h-7 rounded-lg bg-[#09090B] group-hover:bg-[#8B5CF6]/10 transition-colors">
                            {prompt.icon}
                          </span>
              ...
              <div className="p-4.5 border-t border-[#27272A] bg-[#111113] shrink-0">
                <div className="relative flex items-center w-full max-w-3xl mx-auto rounded-2xl border border-[#27272A] bg-[#09090B] shadow-md focus-within:ring-2 focus-within:ring-[#8B5CF6]/15 focus-within:border-[#8B5CF6]/40 transition-all duration-200 p-1.5">
        ...
        <Card className="flex flex-col h-[calc(100vh-140px)] min-h-[400px] border border-[#27272A] shadow-sm overflow-hidden bg-[#111113]/30 backdrop-blur-sm rounded-2xl animate-in fade-in duration-300">
          <CardHeader className="pb-3 pt-4 px-6 border-b border-[#27272A] shrink-0 select-none">
              ...
              <div className="space-y-6 max-w-3xl mx-auto">
                {messages.map((message) => (
                  ...
                  >
                    ...
                    <div
                      className={`p-4.5 rounded-2xl transition-all duration-300 ${
                        message.role === "user"
                          ...
                          : "bg-[#111113] text-card-foreground border border-[#27272A] rounded-tl-none w-full max-w-none shadow-[0_2px_12px_-5px_rgba(0,0,0,0.03)]"
                      }`}
                    >
                    ...
                    {message.role === "user" && (
                      <Avatar className="size-8 shrink-0 shadow-sm border border-[#27272A] select-none">
                        <AvatarFallback className="font-bold bg-[#27272A] text-muted-foreground/90 text-[11px]">U</AvatarFallback>
                      </Avatar>
                    )}
                  </div>
                ))}
